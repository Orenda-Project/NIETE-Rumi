#!/usr/bin/env bash
# Shim for the SHARED E2E engine's e2e-autorun.sh. The engine is authored in rumi-agent-home and linked in at
...
H="$ROOT/.claude/qa/engine/hooks/e2e-autorun.sh"
[ -f "$H" ] || exit 0
exec bash "$H" "$@"
