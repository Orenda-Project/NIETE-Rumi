...
CDP layer, not MCP: `node .claude/qa/engine/bin/flow-drive.js probe|click --match <re>|screen` for the Flow
...
An `all` run's report must account for **all 122** `@e2e` scenarios across its seven features — one
...
python3 .claude/qa/engine/bin/parse-per-scenario.py "$RUN_DIR"
...
`.claude/qa/engine/githooks/post-commit` (installed by `npm install` / `bash .claude/qa/engine/scripts/install-hooks.sh`): it writes a
...
(`.claude/qa/engine/githooks/pre-push`) — see [`docs/qa-automation.md`](../../docs/qa-automation.md).
