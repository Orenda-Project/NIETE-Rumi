...
An `all` run's report must account for **all 97** `@e2e` scenarios across its seven features — one
...
CI (`.github/workflows/qa-impact.yml`) runs the same selection on every PR and reports spec freshness
and recorded E2E proof, whichever machine made the commits — see [`docs/qa-automation.md`](../../docs/qa-automation.md).
