---
name: niete-menu-agent
description: NIETE (ICT) WhatsApp E2E agent for the /menu surface AND the Ask Anything feature (the lightweight 4th menu row lives in menu.feature). Self-contained — resolves the target, loads @e2e scenarios from menu.feature, drives them via Chrome MCP against a linked WhatsApp Web session, and returns JSON. NO prompts.
order: 2
feature: tests/features/whatsapp/niete/menu.feature
---

# NIETE — Menu E2E Agent

Execute `@e2e`-tagged scenarios from **`menu.feature`** against the NIETE (ICT) bot and
...
- **After `/menu` the account sits in a menu-selection state that swallows free text.** Any
  free-text scenario (Ask Anything, a capability question, gibberish) sent in that state gets the
  "choose an option (1-4)" bounce instead of a real answer. Enter Ask Anything FIRST (tap
  View Features → Ask Anything → send), then send the free text. Two scenarios were mis-recorded
  this way before being re-run clean.
- **The ICT menu has exactly 4 rows** — Teacher Training · Lesson Plans · Classroom Coaching ·
  Ask Anything. No Reading Assessment. But the card BODY still advertises "reading assessments" — check the rows, and flag the body copy separately.

## Step 0: Setup
1. **Resolve tenant** — read `@profile:niete` from the `Feature:` line of
   `tests/features/whatsapp/niete/menu.feature` (fallback: `default_profile` in
...
9. **Log** — `[WA-E2E] setup complete — tenant:NIETE env:<env> method:chrome feature:menu`.
...
run — report the real status (a passing `@known-fail` = bug fixed).

## Step 2: Execute Each Scenario
Use the [interaction map](../shared/whatsapp-interaction-map.md) for every send/read/tap/Flow step.
Expected values (menu rows, portal URL) come from
[`../fixtures/whatsapp/niete/answer-keys.yaml`](../fixtures/whatsapp/niete/answer-keys.yaml) — never inline them.
...
9. **On FAIL, emit a drift row.** When an assertion fails, you already have
   ...
   }, ".claude/qa/ledgers/drift/whatsapp/niete/menu.jsonl")
   PY
   ```
...
1. **Covered set.** Build the `covered` list `{kind,label}` from this feature's
   scenarios + `../fixtures/whatsapp/niete/answer-keys.yaml` (e.g. `menu.expected_rows`
   → `{kind:"list-row", label:<row>}`; recognized commands like `/menu`, `/language`).
2. **Existing slugs.** Read `../ledgers/discoveries/whatsapp/niete/menu.md` (if any) and
...
4. **Write entries.** For each `fresh` item, append to
   `../ledgers/discoveries/whatsapp/niete/menu.md`:
   ```markdown
   ### D-niete-menu-<n> · "<label>" <kind>
   - status: proposed
   - slug: <slug>
   ...
   ```
   `<n>` = next integer after the highest existing `D-niete-menu-NNN`. Write ONLY the
...
Expected for this suite: the `@known-fail` `/menu`-body scenario (F1) FAILs until
`whatsapp.service.js:1884` is fixed; `/portal` is `@known-fail` until `PORTAL_URL` is set.
...
ledger.append_run({
  ...
  "env": ENV, "method": "chrome", "feature": "menu",
...
PY
```
`RUN_ID` = `<TS>-niete-menu-<short>`. A runs-append failure is logged, does not
