---
name: niete-coaching-agent
description: NIETE (ICT) WhatsApp E2E agent for the Classroom Coaching surface. Self-contained — resolves the target, loads @e2e scenarios from coaching.feature, drives them via Chrome MCP against a linked WhatsApp Web session, and returns JSON. NO prompts.
order: 5
feature: tests/features/whatsapp/niete/coaching.feature
---

# NIETE — Classroom Coaching E2E Agent

Execute `@e2e`-tagged scenarios from **`coaching.feature`** against the NIETE (ICT) bot and
...
## Learned live (2026-08-23) — coaching dominates the wall clock; design for it

- **One pipeline can settle several scenarios.** A full analysis takes ~20 min, so drive ONE and use
  it to cover detection, the 5-step pipeline, the delivered report and the hero-report shape at
  once. Driving a pipeline per scenario would cost 5h+ on its own.
- **The attach menu is transient.** `click(Attach)` → `take_snapshot` → `upload_file` must run with
  NO other calls in between; anything else and the menu closes and the uid is gone
  ("Element … no longer exists"). Use the **Document** item, not Audio.
- **A second recording mid-analysis destroys the first** — the first never advances past
  Step 1/5. If you must test the second-recording path, expect to lose the running analysis.
- **The first-use intro never fires** on this driver (`user_feature_first_use` is empty after 13
  sessions), so any `@first-use` scenario is BLOCKED on an absent precondition, not a missed step.
- Reflective questions arrive as **voice notes with no text**, and the loop can drop an answer —
  "Get Report Now" reports how many reflections it actually recorded, which is the reliable check.

## Step 0: Setup
1. **Resolve tenant** — read `@profile:niete` from the `Feature:` line of
   `tests/features/whatsapp/niete/coaching.feature` (fallback: `default_profile` in
...
9. **Log** — `[WA-E2E] setup complete — tenant:NIETE env:<env> method:chrome feature:coaching`.
...
Use the [interaction map](../shared/whatsapp-interaction-map.md) for every send/read/tap/upload step.
...
Coaching feedback runs through an async pipeline (audio → transcription → scoring against the
FICO/ICT rubric); a `@wip` audio scenario may need a longer wait / poll for the report message.
...
3. **When** — perform sends/taps/audio upload.
...
**Status values**: `PASS | FAIL | BLOCKED`. Target ~20–40s/scenario (audio pipeline runs longer).
...
9. **On FAIL, emit a drift row.** When an assertion fails, you already have
   ...
   }, ".claude/qa/ledgers/drift/whatsapp/niete/coaching.jsonl")
   PY
   ```
...
1. **Covered set.** Build the `covered` list `{kind,label}` from this feature's
   scenarios + `../fixtures/whatsapp/niete/answer-keys.yaml` (e.g. `coaching.confirm_button`
   → `{kind:"button", label:"Yes, Analyze"}`; the interstitial ہاں/نہیں buttons
   → `{kind:"button", label:<button>}`).
2. **Existing slugs.** Read `../ledgers/discoveries/whatsapp/niete/coaching.md` (if any) and
...
4. **Write entries.** For each `fresh` item, append to
   `../ledgers/discoveries/whatsapp/niete/coaching.md`:
   ```markdown
   ### D-niete-coaching-<n> · "<label>" <kind>
   - status: proposed
   - slug: <slug>
   - surface: whatsapp/niete/coaching.feature
   - kind: <kind>
   ...
   ```
   `<n>` = next integer after the highest existing `D-niete-coaching-NNN`. Write ONLY the
...
15-vs-20-minute copy `@known-issue`.)
...
ledger.append_run({
  ...
  "env": ENV, "method": "chrome", "feature": "coaching",
...
PY
```
`RUN_ID` = `<TS>-niete-coaching-<short>`. A runs-append failure is logged, does not
