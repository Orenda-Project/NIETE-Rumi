...
# -L: copy the engine by value, never the fixture's symlink to the live engine (see githooks.test.sh).
cp -RL "$REAL_ROOT/.claude/qa"   "$R/.claude/qa";    cp -R "$REAL_ROOT/.claude/hooks" "$R/.claude/hooks"
...
HOOK="$R/.claude/qa/engine/hooks/e2e-autorun.sh"; STOP="$R/.claude/qa/engine/hooks/e2e-autorun-stop.sh"
...
(cd "$R" && bash .claude/qa/engine/scripts/install-hooks.sh --quiet >/dev/null 2>&1)
