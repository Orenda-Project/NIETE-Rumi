...
# Every repo-specific fact (deploy branches, spec dir, engine paths) comes from tenants.yaml via lib/tenants.sh (bd-9157r).
...
push_targets_deploy_branch() {
  ...
  # The deploying branches are the repo's `branches.deploy_triggers` in tenants.yaml — NIETE lands on sandbox,
  # the main bot on staging, both release from main. No manifest = fail closed to the two universal names.
  local branches
  branches=$(e2e_get branches.deploy_triggers 2>/dev/null | paste -sd'|' -)
  [ -n "$branches" ] || branches='main|staging'
  printf '%s' "$1" \
    | grep -qE 'git'"$_GIT_OPTS"'[[:space:]]+push[^|;&]*[[:space:]:]('"$branches"')([[:space:]:]|$)'
}
...
e2e_run_selector() {
  ...
  local bin="$(e2e_engine_bin)/select_e2e.py"
...
e2e_run_spec_sync() {
  ...
  local bin="$(e2e_engine_bin)/spec_sync.py"
  command -v python3 >/dev/null 2>&1 || return 1
  [ -f "$bin" ] || return 1
...
e2e_phase1_invalid() {
  ...
  [ -f "$(e2e_engine_bin)/validate_specs.py" ] || return 0
  declared=" $(e2e_phase1_declared "$marker" "$cwd") "
  ...
  [ -n "$feats" ] || return 0
  out=$(cd "$root" && python3 "$(e2e_engine_bin)/validate_specs.py" --only "$feats" 2>&1); rc=$?
  [ "$rc" -eq 0 ] && return 0
