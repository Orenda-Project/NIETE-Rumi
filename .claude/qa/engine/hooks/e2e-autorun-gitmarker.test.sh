...
#
# A terminal commit arms a `git-<sha>.json` marker (see .claude/qa/engine/githooks/post-commit).
...
PROJ="$TMP/proj"; mkdir -p "$PROJ/.claude/.e2e-pending" "$PROJ/.claude/qa/config"
cp -R "$ROOT/.claude/hooks" "$PROJ/.claude/hooks"
# a guarded project = shims + a manifest + the engine (by value, never the fixture's symlink)
cp "$ROOT/.claude/qa/config/tenants.yaml" "$PROJ/.claude/qa/config/tenants.yaml"; cp -RL "$ROOT/.claude/qa/engine" "$PROJ/.claude/qa/engine"
PEND="$PROJ/.claude/.e2e-pending"
STOP="$PROJ/.claude/qa/engine/hooks/e2e-autorun-stop.sh"
BANNER="$PROJ/.claude/qa/engine/hooks/e2e-pending-banner.sh"
S="sess-$$"
...
bash "$PROJ/.claude/qa/engine/hooks/e2e-autorun.sh" --clear --session git-bbb222 2>/dev/null
...
# A real clone: git repo + .claude/qa/engine/githooks + the installer, nothing pending. The banner must
# leave it with core.hooksPath=.claude/qa/engine/githooks and SAY it did — a developer who never ran
# `npm install` had no hooks, and nothing told them.
G="$TMP/clone with space"; mkdir -p "$G/.claude" "$G/scripts"
# a guarded clone = the hook shims + a manifest + the engine (copied by value, never the fixture's symlink)
cp -R "$ROOT/.claude/hooks" "$G/.claude/hooks"; mkdir -p "$G/.claude/qa/config"
cp "$ROOT/.claude/qa/config/tenants.yaml" "$G/.claude/qa/config/tenants.yaml"; cp -RL "$ROOT/.claude/qa/engine" "$G/.claude/qa/engine"
...
gbanner() { printf '{"session_id":"%s","cwd":"%s","hook_event_name":"SessionStart"}' "$S" "$G" | CLAUDE_PROJECT_DIR="$G" bash "$G/.claude/qa/engine/hooks/e2e-pending-banner.sh" 2>/dev/null; }
say "fixture starts with no hooksPath" "$(git -C "$G" config --get core.hooksPath)" ""
out=$(gbanner)
say "SessionStart sets core.hooksPath=.claude/qa/engine/githooks" "$(git -C "$G" config --get core.hooksPath)" ".claude/qa/engine/githooks"
...
[ -x "$G/.claude/qa/engine/githooks/post-commit" ] && ok "post-commit is executable afterwards" || bad "post-commit not executable"
out=$(gbanner)
