...
ARM=".claude/qa/engine/hooks/e2e-autorun.sh"
STOP=".claude/qa/engine/hooks/e2e-autorun-stop.sh"
...
# adopts an un-nudged `git-<sha>.json` left by a terminal commit (.claude/qa/engine/githooks/
...
. .claude/qa/engine/hooks/lib/git-push-match.sh
...
has "commit: names the build it hits" "$COUT" "after the sandbox deploy" yes   # the manifest's landing branch (was the retired `develop`)
...
for GREPBIN in /usr/bin/grep /opt/homebrew/bin/grep /usr/local/bin/grep \
  ...
  LIB="$PWD/.claude/qa/engine/hooks/lib/git-push-match.sh"
...
BADRE=$(grep -v '^[[:space:]]*#' .claude/qa/engine/hooks/lib/git-push-match.sh \
        | grep -c '(\\b|' || true)
...
cleanup
BROKEN=$(mktemp -d)
# -L: the fixture's .claude/qa/engine is a SYMLINK to the live engine — copy by value, or the next line
# overwrites the REAL spec_sync.py (it did, twice, on 2026-09-17).
cp -RL .claude "$BROKEN/.claude" 2>/dev/null
[ -L "$BROKEN/.claude/qa/engine" ] && { echo "refusing: engine copy is still a symlink"; exit 1; }
echo 'import sys; sys.exit(1)' > "$BROKEN/.claude/qa/engine/bin/spec_sync.py"
# The engine resolves its own bin/ relative to the hook that runs, so the BROKEN copy's hook must be the one invoked
# (in a real repo the running hook IS the vendored engine — same thing).
OUT=$(CLAUDE_PROJECT_DIR="$BROKEN" ARM="$BROKEN/.claude/qa/engine/hooks/e2e-autorun.sh" arm "$G commit -m 'change the menu'")
