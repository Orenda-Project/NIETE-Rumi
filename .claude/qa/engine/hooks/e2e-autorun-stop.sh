...
# Not a guarded repo (no tenants.yaml above CLAUDE_PROJECT_DIR or the session cwd): nothing to enforce.
...
nothing. Do ONE of these, then end the turn:
  ...
  python3 .claude/qa/engine/bin/validate_specs.py --only $FEATS

  bash .claude/qa/engine/hooks/e2e-autorun.sh --declare --session $CLEAR_ID '$FEATS=none-needed (<why>)'
...
reads commits, an edit left in the working tree does not count:
  git add $E2E_SPEC_DIR/<feature>.feature $E2E_CMD_DOC
...
alters nothing a teacher sees, declare it instead of skipping it:
  bash .claude/qa/engine/hooks/e2e-autorun.sh --declare --session $CLEAR_ID '<feature>=none-needed (<why>)'
...
WhatsApp Web run for this commit. The chrome lane ($E2E_CMD) tests it
after the develop deploy, when the build is actually live.
EOF
elif [ "$TRIGGER" = "commit" ]; then
  read -r -d '' BUILD_NOTE <<EOF
⚠ THE $E2E_CMD PART CANNOT TEST WHAT WAS JUST COMMITTED. The commit has not been
...
EOF
  ...
  read -r -d '' CHROME_PRE <<EOF
The mock lane needs no browser and no WhatsApp number: ${E2E_KEYS_LOCAL:-the keys file named in tenants.yaml} and
...
Clear it either way when you are done:
  bash .claude/qa/engine/hooks/e2e-autorun.sh --clear --session $CLEAR_ID
