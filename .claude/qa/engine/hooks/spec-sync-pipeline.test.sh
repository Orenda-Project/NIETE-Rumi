...
# -L: copy the engine BY VALUE. The fixture's .claude/qa/engine is a symlink to the live engine and the
# "broken sync" stage below overwrites spec_sync.py in the copy — through a symlink that would be the real file.
cp -RL "$REAL_ROOT/.claude/qa"   "$PROJ/.claude/qa"
...
print(json.dumps({'session_id':'$SESSION','cwd':'$REPO','tool_name':'Bash',
 ...
 'tool_response':{'stdout':'','stderr':'','interrupted':False}}))" \
  | CLAUDE_PROJECT_DIR="$PROJ" bash "$PROJ/.claude/qa/engine/hooks/e2e-autorun.sh" 2>/dev/null
}
...
print(json.dumps({'session_id':'$SESSION','cwd':'$REPO','hook_event_name':'Stop'}))" \
  | CLAUDE_PROJECT_DIR="$PROJ" bash "$PROJ/.claude/qa/engine/hooks/e2e-autorun-stop.sh" 2>/dev/null
}
...
validate() { python3 "$PROJ/.claude/qa/engine/bin/validate_specs.py" \
...
MENU_COUNT=$(python3 "$PROJ/.claude/qa/engine/bin/parse-gherkin.py" "$PROJ/tests/features/whatsapp/niete/menu.feature" 2>/dev/null | python3 -c 'import json,sys; print(json.load(sys.stdin)["count"])' 2>/dev/null || echo 13)
...
echo 'import sys; sys.exit(1)' > "$PROJ/.claude/qa/engine/bin/spec_sync.py"
