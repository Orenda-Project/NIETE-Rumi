...
# Clear a pending marker: bash .claude/qa/engine/hooks/e2e-autorun.sh --clear [--session ID]
  ...
  if [ "$FORCE" != 1 ] && command -v e2e_phase1_stale >/dev/null 2>&1; then
    for M in $TARGETS; do
      ...
      if [ -n "$STALE" ]; then
        ...
        echo "  or declare:  bash .claude/qa/engine/hooks/e2e-autorun.sh --declare --session $ID '$(printf '%s' "$STALE" | tr ' ' ',')=none-needed (<why>)'" >&2
...
# Not a guarded repo (no .claude/qa/config/tenants.yaml above CLAUDE_PROJECT_DIR or the command's cwd): silent.
e2e_root "$PAYLOAD_CWD" >/dev/null 2>&1 || exit 0
E2E_CMD=$(e2e_cmd); E2E_CMD_DOC=$(e2e_get command_doc); E2E_KEYS_LOCAL=$(e2e_get keys.local); E2E_SPEC_DIR=$(e2e_spec_dir)
E2E_LANDING=$(e2e_get branches.landing)
...
if [ "${E2E_AUTORUN_ALL:-}" = "1" ]; then
  ...
  SEL='{"commands":["'"$E2E_CMD"' all"],"features":["all"],"fallback":false,"unmapped":[]}'
else
...
[ -z "$BRANCH" ] && BRANCH=$(printf '%s' "$SCAN" | grep -oE "($(e2e_get branches.deploy_triggers | paste -sd'|' -))" | head -1)
...
printf '%s' "$SEL" | jq \
  ...
  '{session: $session, repo: $repo, branch: $branch, commit_sha: $commit_sha, trigger: $trigger, mode: $mode,
    ...
    commands: .commands, features: .features, tenants: (.tenant_features // {}),
...
if [ "$TRIGGER" = "commit" ] && [ -n "$COMMIT_SHA" ] && type e2e_split_lanes >/dev/null 2>&1; then
  ...
  if [ -n "$E2E_LANE_MOCK" ]; then
    # several tenants → one commit-e2e line per tenant; one tenant → today's single line, unchanged
...
WhatsApp Web run for this commit; $E2E_CMD tests it after the $E2E_LANDING deploy.
EOF
elif [ "$TRIGGER" = "commit" ]; then
  read -r -d '' WHICH_BUILD <<EOF
⚠ THE $E2E_CMD PART CANNOT TEST WHAT YOU JUST COMMITTED. The commit has not been
...
feature, update the per-feature counts in $E2E_CMD_DOC too —
...
Preconditions for the WhatsApp Web lane — check, do not skip:
  ...
  · The Chrome DevTools MCP. If \`list_pages\` is an unknown tool, the server is
    not loaded — and that is INSTALLABLE, not a dead end. \`$E2E_CMD\` says
...
EOF
  ...
  read -r -d '' CHROME_PRE <<EOF
The mock lane needs no browser and no WhatsApp number: ${E2E_KEYS_LOCAL:-the keys file named in tenants.yaml} and installed
...
plainly, then clear the marker:
  bash .claude/qa/engine/hooks/e2e-autorun.sh --clear --session $SESSION
