...
set -uo pipefail
# Engine scripts live beside this file (…/.claude/qa/engine/bin); the REPO is the one carrying tenants.yaml.
QA="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=../hooks/lib/tenants.sh
. "$QA/../hooks/lib/tenants.sh" 2>/dev/null || { echo "ERROR: cannot load $QA/../hooks/lib/tenants.sh"; exit 2; }
...
# whatsapp-targets.yaml profile on the chrome lane.
[ -n "$TENANT" ] || TENANT=$(e2e_tenants | head -1)
...
# profiles are small YAML maps: find this tenant's profile whose env matches, without needing PyYAML
for block in re.split(r"\n  (?=[a-z][\w-]*:\s*\n)", txt):
    if re.search(r"env:\s*%s\b" % env, block) and match in block:
...
say() { echo "$*" | tee -a "$LOG"; }
T0=$(date +%s)
say "=== $E2E_CMD $MODE · $(date -u +%FT%TZ) · tenant $TENANT env=$ENV target=$TARGET driver=$DRIVER method=$METHOD${COMMIT:+ commit=${COMMIT:0:12}} run=$RUN_ID"
...
STACK_DOWN=""
cleanup() { python3 "$QA/driver_lock.py" release --driver "$DRIVER" >/dev/null 2>&1; [ -n "$STACK_DOWN" ] && bash "$E2E_SCRIPTS/local-stack.sh" down "$RUN_DIR" >>"$LOG" 2>&1; }
trap cleanup EXIT
if [ "$METHOD" = mock ]; then
  ...
  STACK_DOWN=1
  bash "$E2E_SCRIPTS/local-stack.sh" up "$COMMIT" "$RUN_DIR" >>"$LOG" 2>&1 || { rc=$?; say "BLOCKED: local stack did not come up (exit $rc) — see $LOG, $RUN_DIR/bot.log"; exit 3; }
...
root, rel = sys.argv[1], sys.argv[2] or "keys/local.env"
...
for c in (os.path.join(main, rel), os.path.join(main, "keys", name), os.path.join(os.path.dirname(main), "keys", name)):
    if os.path.isfile(c): print(c); break
PY
)
  [ -f "$KEYS_FILE" ] && eval "$(grep -E '^SUPABASE_(URL|SERVICE_ROLE_KEY)=' "$KEYS_FILE" | sed "s/^SUPABASE_/export ${DB_PREFIX}_/")"
  ...
  run_tool sandbox_driver ensure --phone "$DRIVER" --yes-write 2>&1 | tee -a "$LOG" | tail -1
else
  ...
  case "$HEADER" in *Staging*|*"$TMATCH"*|*"$TARGET"*) say "chat open: $HEADER";; *) say "BLOCKED: the target chat is not open in WhatsApp Web (header: '${HEADER:-none}'). Open the bot chat ($TARGET) with a real click, confirm the header, re-run."; exit 3;; esac
fi
...
if [ "$MODE" = "all" ] || echo "$FEATURES" | grep -qx coaching; then
  run_tool coaching_db cancel-stuck --env "$ENV" --phone "$DRIVER" --yes-write 2>&1 | tee -a "$LOG" | tail -1
  ...
  if should_reset_history "$METHOD" "$MODE" "${E2E_CASSETTE:-off}"; then
    run_tool coaching_db reset-history --env "$ENV" --phone "$DRIVER" --yes-write 2>&1 | tee -a "$LOG" | tail -1
  fi
  ...
  if [ "${FIRSTUSE:-}" = "1" ] || { [ "$METHOD" = mock ] && [ "${FIRSTUSE:-1}" = "1" ]; }; then
    run_tool coaching_db reset-first-use --env "$ENV" --phone "$DRIVER" --yes-write 2>&1 | tee -a "$LOG" | tail -1
  fi
fi
...
reset_state() {
  if [ "$METHOD" = mock ]; then run_tool sandbox_driver reset-state --phone "$DRIVER" --yes-write >>"$LOG" 2>&1
...
for f in $FEATURES; do
  case "$f" in
    coaching)
      ...
      run_tool coaching_db cancel-stuck --env "$ENV" --phone "$DRIVER" --yes-write >>"$LOG" 2>&1 || true
      ...
      if should_reset_history "$METHOD" "$MODE" "${E2E_CASSETTE:-off}"; then
        run_tool coaching_db reset-conversations --env "$ENV" --phone "$DRIVER" --yes-write >>"$LOG" 2>&1 || true
      fi
    ...
    training)
      if [ "$METHOD" = mock ]; then
        ...
        PROGRAM_KEY=$(e2e_get tenant_tools.training_program); PROGRAM_KEY="${PROGRAM_KEY:-standard}"
        say "seed(mock): activate-program $PROGRAM_KEY + revert-level 1 on $DRIVER ($ENV)"
        run_tool training_db activate-program --env "$ENV" --phone "$DRIVER" --program-key "$PROGRAM_KEY" --yes-write >>"$LOG" 2>&1 || say "seed(mock): activate-program failed (see runner.log)"
        run_tool training_db revert-level --env "$ENV" --phone "$DRIVER" --level 1 --yes-write >>"$LOG" 2>&1 || say "seed(mock): revert-level failed (see runner.log)"
        ...
        run_tool training_db revert-level --env "$ENV" --phone "$DRIVER" --level 1 --yes-write >>"$LOG" 2>&1
        run_feature training
        say "seed: seed-level-complete 1 — restoring the account"
        run_tool training_db seed-level-complete --env "$ENV" --phone "$DRIVER" --level 1 --yes-write >>"$LOG" 2>&1
    ...
    registration)
      if [ "$METHOD" = mock ]; then
        ...
        run_tool sandbox_driver unregister --phone "$DRIVER" --yes-write >>"$LOG" 2>&1 || true
        run_feature registration
        run_tool sandbox_driver ensure --phone "$DRIVER" --yes-write >>"$LOG" 2>&1 || true
...
say "next: python3 $QA/validate-run.py \"$RUN_DIR\" · python3 $QA/run_efficiency.py \"$RUN_DIR\" · then §3 of $E2E_CMD"
