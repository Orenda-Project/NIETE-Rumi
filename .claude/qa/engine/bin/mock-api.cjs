...
function makeMockApi(opts) {
  ...
  // The repo is the one carrying tenants.yaml; the tenant fixes the default driver, the DB tools and the DB env
  // prefix (bd-9157r). Feature scripts read `api.tenant` instead of naming a tenant.
  const T = require(path.join(__dirname, 'tenant.cjs')).load(opts.repo);
  const repo = T.root;
  const tenant = T.tenant((opts.tenant || process.env.E2E_TENANT || '').trim() || Object.keys(T.tenants)[0]);
  const driver = opts.driver || process.env.E2E_DRIVER || tenant.driver;
  ...
  const { createEmulator, FlowTransport } = require(path.join(repo, T.e2eScripts, 'flow-emulator.js'));
  ...
  const endpointFor = (envVar) => {
    ...
    if (!endpointsByEnv) {   // the bot's own registry: flow-configs.js (envVar → endpointPath)
      endpointsByEnv = {};
      try { const cfg = require(path.join(repo, T.botRoot, 'scripts', 'setup', 'flow-configs.js')); const list = Array.isArray(cfg) ? cfg : (cfg.FLOW_CONFIGS || cfg.flows || Object.values(cfg).find(Array.isArray) || []);
        for (const f of list) if (f && f.envVar && f.endpointPath) endpointsByEnv[f.envVar] = f.endpointPath; } catch (_) { /* no registry */ }
      try { routesSource = fs.readFileSync(path.join(repo, T.botRoot, 'shared', 'routes', 'flow-endpoint.routes.js'), 'utf8'); } catch (_) { routesSource = ''; }
    }
  ...
  return {
    tenant,   // { id, region_code, phone_number_id, driver, features, chrome, language, copy } — feature scripts read this, never a literal
