...
set -u
cd "$(dirname "$0")/../../../.." || exit 1
. .claude/qa/engine/hooks/lib/tenants.sh; TEN=$(e2e_tenants | head -1)
FAILED=0
ok()  { printf '  ok    %s\n' "$1"; }
bad() { printf '  FAIL  %s\n' "$1"; FAILED=$((FAILED + 1)); }
say() { [ "$2" = "$3" ] && ok "$1" || bad "$1 (got '$2', want '$3')"; }
RS=.claude/qa/engine/bin/run-suite.sh
...
want=$(E2E_MOCK_DRIVER_MACHINE="alpha.local|mac" python3 .claude/qa/engine/bin/mock_driver.py $MDT)
