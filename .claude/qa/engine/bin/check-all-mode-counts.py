...
# The command doc and the spec dir come from the repo's tenants.yaml (bd-9157r). A repo whose slash command
# has no doc yet (the main bot, until its tenant layer lands) has nothing for this checker to compare, so
# it says so and exits 0 rather than failing on a file that is not supposed to exist.
_M = _tl.load(_tl.repo_root())
ROOT = _M.root
CMD = os.path.join(ROOT, _M.command_doc)
FEATURES = os.path.join(ROOT, _M.spec_dir)
