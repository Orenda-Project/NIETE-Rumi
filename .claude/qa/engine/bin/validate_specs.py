...
def _agent_file(agents_dir, feature):
    """`<feature>-agent.md`, or the historical `<prefix>-<feature>-agent.md` (one hyphen-free prefix,
    e.g. `niete-menu-agent.md`), else None. Anchored so `plan` never matches `lesson-plan-agent.md`."""
    direct = os.path.join(agents_dir, "%s-agent.md" % feature)
    if os.path.isfile(direct):
        return direct
    pat = _re.compile(r"^[a-z0-9]+-%s-agent\.md$" % _re.escape(feature))
    try:
        hits = sorted(f for f in os.listdir(agents_dir) if pat.match(f))
    except OSError:
        hits = []
    return os.path.join(agents_dir, hits[0]) if hits else None


_TENANTS_TAG = _re.compile(r"@tenants:([A-Za-z0-9_,-]+)")
...
def validate_file(path, agents, tenants=None):
    """Return a list of Problems for one .feature file.

    `agents` is a directory of `<feature>-agent.md` (or `<prefix>-<feature>-agent.md`) files, or a
    set/list of bare feature names (the tests pass a directory). `tenants` is the set of tenant ids
    from tenants.yaml; when given, every `@tenants:a,b` restriction must name only known ids.
    """
    problems = []
    name = os.path.basename(path)[:-len(".feature")]

    if isinstance(agents, str):
        have_agent = _agent_file(agents, name) is not None
    else:
        have_agent = name in set(agents)
    if not have_agent:
        problems.append(Problem("error", "E-NOAGENT", path, 1,
                                "no %s-agent.md (or <prefix>-%s-agent.md) in the agents dir — nothing "
                                "will ever run this spec. Add the agent, or the file is dead weight."
                                % (name, name)))
    ...
    if tenants is not None:
        ...
        with open(path, encoding="utf-8") as fh:
            for ln, text in enumerate(fh, 1):
                for m in _TENANTS_TAG.finditer(text):
                    for tid in m.group(1).split(","):
                        if tid and tid not in known:
                            problems.append(Problem("error", "E-TENANT-UNKNOWN", path, ln,
                                                    "@tenants names %r, which is not a tenant in "
                                                    ".claude/qa/config/tenants.yaml (known: %s). A "
                                                    "misspelt tenant silently drops the scenario from "
                                                    "every run." % (tid, ", ".join(sorted(known)))))
...
def validate_dir(spec_dir, agents, only=None, tenants=None):
    ...
    for path in sorted(glob.glob(os.path.join(spec_dir, "*.feature"))):
        ...
        if only and name not in set(only):
            continue
        problems += validate_file(path, agents, tenants=tenants)
    return problems
...
def run(spec_dir, agents, only=None, as_json=False, stream=None, tenants=None):
    stream = stream or sys.stdout
    problems = validate_dir(spec_dir, agents, only=only, tenants=tenants)
    if as_json:
...
def main(argv):
    # Defaults come from the repo's tenants.yaml (CLAUDE_PROJECT_DIR or the cwd's ancestors), never from
    ...
    if not a.spec_dir:
        print("[validate-specs] cannot decide: no tenants.yaml found and no --spec-dir given", file=sys.stderr)
    ...
    return run(a.spec_dir, a.agents_dir, only=only or None, as_json=a.json, tenants=tenants)
