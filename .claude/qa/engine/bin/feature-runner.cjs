...
// The REPO is the one carrying tenants.yaml (CLAUDE_PROJECT_DIR › cwd's ancestors › the engine's vendored parent),
// never this file's own ancestors — those are .claude/qa/engine/bin. The tenant fixes the default driver, the
// phone-number id the mock lane boots with, the DB tools and the results dir (bd-9157r).
const T = require(path.join(__dirname, 'tenant.cjs')).load();
const REPO = T.root;
const TENANT = T.defaultTenant();
...
function makeApi(c) {
  ...
  return {
    ...
    db(action, extra) {
      trace('db ' + action);
      const toolName = /^(lookup|answer-key|module-answer-key)$/.test(action) ? 'training_db' : 'registration_db';
      const script = T.tool(toolName);
      if (!script) return { ok: false, err: 'TENANT_TOOL_UNCONFIGURED: tenant_tools.' + toolName + ' is not set in tenants.yaml (or the file is missing)' };
      const args = [script, action, '--env', ENV, '--phone', process.env.E2E_DRIVER || TENANT.driver];
     ...
     *  bot's in-process history Map (via POST /clear-history/<uid>) — the DB alone is not enough
     *  because getConversationHistory is cache-first. Call it right before the scenario. */
    resetConversation() {
      trace('resetConversation');
      const botUrl = process.env.E2E_BOT_URL || `http://127.0.0.1:${process.env.E2E_BOT_PORT || 3100}`;
      const tool = T.tool('coaching_db');
      if (!tool) return { ok: false, err: 'TENANT_TOOL_UNCONFIGURED: tenant_tools.coaching_db is not set in tenants.yaml' };
      const args = [tool, 'reset-conversations',
        '--env', ENV, '--phone', process.env.E2E_DRIVER || TENANT.driver, '--yes-write', '--clear-cache-url', botUrl];
    ...
    async setUser(patch) {
      ...
      if (!url || !key) return { ok: false, err: 'no E2E DB creds (' + prefix + '_URL / ' + prefix + '_SERVICE_ROLE_KEY)' };
      try {
        const res = await fetch(`${url}/rest/v1/users?phone_number=eq.${process.env.E2E_DRIVER || TENANT.driver}`, { method: 'PATCH',
...
(async () => {
  ...
  const flows = MOCK && process.env.E2E_FLOWS_DIR && process.env.E2E_FLOW_PUBLIC_KEY_B64
    ...
    : null;
  const api = MOCK
    ? require(path.join(__dirname, 'mock-api.cjs')).makeMockApi({ baseUrl: process.env.E2E_MOCK_URL, driver: process.env.E2E_DRIVER || TENANT.driver, env: ENV, repo: REPO, tenant: TENANT.id, trace, flows })
    : makeApi(c);
  // Feature scripts read the tenant (id, region_code, language, copy fixture) instead of naming one.
  if (!api.tenant) api.tenant = TENANT;
  const results = [];
  ...
  const _dbe = T.dbEnv();
  const _SB = { url: _dbe.url, key: _dbe.key, drv: process.env.E2E_DRIVER || TENANT.driver };
  ...
  try {
    ...
    // Drivers are TENANT LAYER: <repo>/<drivers_dir>/<feature>.cjs, never a directory beside this vendored file.
    const mod = require(path.join(REPO, T.driversDir, FEATURE + '.cjs'));
    await mod.run({ api, rec, sleep, tenant: TENANT });
  } catch (e) {
