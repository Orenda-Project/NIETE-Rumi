...
(async () => {
  ...
  await ita('api.tenant carries the manifest row (id, driver) and the copy fixture', async () => {
    assert.ok(api.tenant && api.tenant.id, 'api.tenant.id');
    assert.ok(Array.isArray(api.tenant.features), 'features list');
    assert.strictEqual(typeof api.tenant.copy, 'object', 'copy fixture object (empty when no copy.yaml)');
  });
