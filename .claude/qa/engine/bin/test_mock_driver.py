...
Run: python3 .claude/qa/engine/bin/test_mock_driver.py
"""
...
def _run(*args, **env):
    e = dict(os.environ); e.pop("E2E_MOCK_DRIVER", None); e.update(env)
    return subprocess.run([sys.executable, SCRIPT, *args], env=e, capture_output=True, text=True).stdout.strip()

# A multi-tenant repo folds the tenant id into the machine key: one row per tenant per machine, and the
# tenant-less number (single-tenant repos, and every number derived before this) is unchanged.
_base = _run(E2E_MOCK_DRIVER_MACHINE="alpha.local|mac")
_pk = _run("--tenant", "pk", E2E_MOCK_DRIVER_MACHINE="alpha.local|mac")
_tz = _run("--tenant", "tz", E2E_MOCK_DRIVER_MACHINE="alpha.local|mac")
check("--tenant changes the number", _pk != _base, True)
check("two tenants on one machine get two numbers", _pk != _tz, True)
check("--tenant is deterministic", _run("--tenant", "pk", E2E_MOCK_DRIVER_MACHINE="alpha.local|mac") == _pk, True)
check("a tenant number still has the 92300 prefix and 12 digits", bool(re.fullmatch(r"92300\d{7}", _pk)), True)
check("E2E_MOCK_DRIVER pins regardless of --tenant", _run("--tenant", "pk", E2E_MOCK_DRIVER="923000000777"), "923000000777")

print(); print("%d test(s), %d failed" % (22, fails)); sys.exit(1 if fails else 0)
