#!/bin/bash
# Tests for the distributed git hooks (.claude/qa/engine/githooks/) and their installer.
#
...
# throwaway repo that carries COPIES of .claude/qa, .claude/qa/engine/githooks and scripts/qa —
# the real repo is never committed to.
#
# Run:  bash .claude/qa/engine/githooks/githooks.test.sh
set -u
cd "${E2E_FIXTURE:?run via tests/run-all.sh with E2E_FIXTURE set}" || exit 1
...
# -L: the fixture's .claude/qa/engine is a SYMLINK to the live engine; copy it by value so nothing this test
# writes into the throwaway repo can reach the real files (a test once overwrote spec_sync.py through it).
cp -RL "$ROOT/.claude/qa"   "$R/.claude/qa"
cp -R  "$ROOT/.claude/hooks" "$R/.claude/hooks"
...
out=$(cd "$R" && bash .claude/qa/engine/scripts/install-hooks.sh 2>&1); rc=$?
say "installer exits 0" "$rc" "0"
say "core.hooksPath points at .claude/qa/engine/githooks" "$(git -C "$R" config core.hooksPath)" ".claude/qa/engine/githooks"
out=$(cd "$R" && bash .claude/qa/engine/scripts/install-hooks.sh --quiet 2>&1); rc=$?
...
(cd "$R" && bash .claude/qa/engine/scripts/install-hooks.sh --force >/dev/null 2>&1)
say "--force takes it over" "$(git -C "$R" config core.hooksPath)" ".claude/qa/engine/githooks"
[ -x "$R/.claude/qa/engine/githooks/post-commit" ] && ok "post-commit is executable" || bad "post-commit is not executable"
[ -x "$R/.claude/qa/engine/githooks/pre-push" ]   && ok "pre-push is executable"   || bad "pre-push is not executable"
...
# The throwaway repo has no keys file (keys.local) and (see the PATH below) no redis-server, so the hook
...
# The provisioner is a BOT SEAM (<e2e_scripts>/provision-local-keys.sh, per repo), so this case runs only in a
# checkout that carries one AND names its sandbox project ref (E2E_PROVISION_SANDBOX_REF); the engine's own
# fixtures do not, and say so instead of failing.
_SCRIPTS=$(cd "$ROOT" && . .claude/qa/engine/hooks/lib/tenants.sh 2>/dev/null && e2e_get e2e_scripts 2>/dev/null); _SCRIPTS="${_SCRIPTS:-scripts/e2e}"
_KEYS=$(cd "$ROOT" && . .claude/qa/engine/hooks/lib/tenants.sh 2>/dev/null && e2e_get keys.local 2>/dev/null); _KEYS="${_KEYS:-keys/local.env}"
if [ -f "$ROOT/$_SCRIPTS/provision-local-keys.sh" ] && [ -n "${E2E_PROVISION_SANDBOX_REF:-}" ]; then
mkdir -p "$TMP/rbin" "$R/$_SCRIPTS"; cp "$ROOT/$_SCRIPTS/provision-local-keys.sh" "$R/$_SCRIPTS/"
SBX="$E2E_PROVISION_SANDBOX_REF"
...
[ -f "$R/$_KEYS" ] && ok "…and it exists under the repo's keys/" || bad "keys file not created by the hook"
has "…so the not-runnable warning is gone" "$err" "railway login" no
else
  ok "auto-provision case skipped — needs $_SCRIPTS/provision-local-keys.sh in this checkout and E2E_PROVISION_SANDBOX_REF"
fi
...
out=$(cd "$R" && printf 'refs/heads/develop %s refs/heads/develop %s\n' "$head" "$base" | bash .claude/qa/engine/githooks/pre-push origin https://example/x.git 2>&1); rc=$?
...
out=$(cd "$R" && printf 'refs/heads/develop %s refs/heads/develop %s\n' "$head" "$zero" | bash .claude/qa/engine/githooks/pre-push origin x 2>&1); rc=$?
