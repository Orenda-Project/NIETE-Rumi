#!/bin/bash
# install-hooks.sh — point git at the repo's distributed hooks (.claude/qa/engine/githooks/).
#
# Git will not run hooks from a tracked directory on its own: .claude/qa/engine/githooks/ travels
...
[ -d "$ROOT/.claude/qa/engine/githooks" ] || exit 0
cd "$ROOT" || exit 0
CUR=$(git config --get core.hooksPath 2>/dev/null)

if [ "$UNINSTALL" = "1" ]; then
  if [ "$CUR" = ".claude/qa/engine/githooks" ]; then git config --unset core.hooksPath; say "qa hooks: uninstalled (core.hooksPath cleared)"; fi
  exit 0
fi
chmod +x .claude/qa/engine/githooks/post-commit .claude/qa/engine/githooks/pre-push 2>/dev/null
if [ "$CUR" = ".claude/qa/engine/githooks" ]; then
  say "qa hooks: already installed (core.hooksPath = .claude/qa/engine/githooks)"
  exit 0
fi
if [ -n "$CUR" ] && [ "$FORCE" != "1" ]; then
  echo "qa hooks: NOT installed — core.hooksPath is already '$CUR' (not ours)." >&2
  echo "          Re-run with --force to take it over, or call .claude/qa/engine/githooks/post-commit and" >&2
  echo "          .claude/qa/engine/githooks/pre-push from your own hooks. Nothing was changed." >&2
  exit 0
fi
git config core.hooksPath .claude/qa/engine/githooks || exit 0
say "qa hooks: installed (core.hooksPath = .claude/qa/engine/githooks) — commits now arm the targeted E2E + Gherkin sync"
exit 0
