...
| `qa/engine/` | **The vendored E2E engine — READ-ONLY here.** Hooks (`hooks/e2e-autorun.sh` · `e2e-autorun-stop.sh` · `e2e-pending-banner.sh` · `qa-engine-guard.sh`), git hooks (`githooks/`), selector, spec-sync brief builder, validator, impact, runner, mock driver, ledgers. Authored in `rumi-agent-home/.claude/qa/engine` and ported here; every repo-specific fact comes from `qa/config/tenants.yaml`. A commit arms the Gherkin sync + targeted E2E for the features it touched; Stop holds the turn once; SessionStart announces markers left by terminal commits. See [qa/engine/README.md](qa/engine/README.md) · [docs/qa-automation.md](../docs/qa-automation.md) |
| `qa/` (everything else) | **This repo's tenant layer**: `config/tenants.yaml` (the one file that names this tenant), `config/feature-map.yaml`, `whatsapp-targets.yaml`, `known-findings.json`, one executor agent per feature, the drivers under `shared/features/`, the `niete_*` seed/reset tools, fixtures, ledgers — [qa/README.md](qa/README.md) |
