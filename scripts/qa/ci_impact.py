#!/usr/bin/env python3
"""ci_impact — the repository-wide QA check for a commit range.

This is the layer that does not care which machine made the commit. GitHub runs
it on every pull request into develop/main and every push to develop
(.github/workflows/qa-impact.yml); the pre-push git hook runs the same code
locally over the commits a push is about to send. Same answer both places.
...
WhatsApp (a linked browser session is a human precondition). CI cannot own
either, so CI's job is to make it impossible to MERGE without them having
happened — or without someone saying, in the commit, why they were not needed.

Modes, per question, from --freshness / --proof (or the QA_SPEC_FRESHNESS /
QA_E2E_PROOF repository variables in CI):  off | warn | block.  `warn` reports
and exits 0; `block` exits 1 on `stale` / `missing`. Ship in warn, promote to
block once the team has lived with the comments for a while — the same
warn-then-block path this repo used for its other gates.

An undecidable range (unknown sha, shallow clone) is REPORTED and exits 0: it is
not evidence against the PR.

    python3 scripts/qa/ci_impact.py --repo . --base <sha> --head <sha> [--pr-body-file f]
                                    [--target-branch develop] [--format md|text|json]
                                    [--freshness warn] [--proof warn] [--md-out f] [--json-out f]
Tests: python3 scripts/qa/test_ci_impact.py
"""
import argparse
import json
import os
...
def render_markdown(res, freshness="warn", proof="warn"):
    ...
    if v["spec_freshness"] == "stale":
        ...
        L += ["### Sync the specs first",
              ...
              "If the change genuinely alters no teacher-visible behaviour, say so where CI can read it — a commit trailer",
...
def render_text(res, freshness="warn", proof="warn"):
    ...
    L.append("└ CI runs this same check on the PR (.github/workflows/qa-impact.yml)")
    return "\n".join(L)
