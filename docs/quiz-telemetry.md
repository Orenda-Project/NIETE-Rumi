...
| `generated` | `process()`, status `ready` | `n` (questions); `cached:true` + `donor_quiz_id` when the lesson-plan cache reused a shipped quiz (no model call) | `status='ready'`, `meta.ready_at`, `meta.question_count`, `quiz_questions` rows; `meta.cache_donor {quiz_id, key, authored_at}` on a cached one | `transcript_quiz.ready` (+ `cached`, `donor_quiz_id`) |
