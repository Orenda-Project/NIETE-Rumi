...
-- users.teacher_level: the ONE place a teacher's band lives, and the teacher's
-- OWN statement of it, chosen on the bot or portal. TRAINING SCOPING ONLY: it
-- maps to training_programs and must NEVER gate lesson plans or any other
-- feature (a teacher picking "Middle" to reach the Beacon House training must
-- not thereby change their lesson-plan content).
--
-- bd-60095 renamed this from training_bands and DROPPED users.levels and
-- users.grade. `levels` had 8,708 rows and not one reader or writer, and it
-- disagreed with the live column for 233 teachers — the role-backfill heuristic
-- that once justified keeping it now reads teacher_level instead. Read this
-- column only through teacherLevelOf() (bot/shared/utils/teacher-level.js):
-- one column, no fallback, every band rather than the first.
ALTER TABLE users ADD COLUMN IF NOT EXISTS teacher_level VARCHAR(16)[];
-- When the teacher last CHANGED teacher_level themselves. NULL = never
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS teacher_level_updated_at TIMESTAMPTZ;
...
-- ============================================================
-- K-5 v8 lesson-plan assets + download tracking (migration 018)
-- Mirrors bot/database/migrations/018_niete_lp_assets_and_downloads.sql.
...
-- ============================================================================
-- Assessment Generator — what a teacher asked for, and what came
-- back. Two tables because the request is fully known at submit time and the
-- paper is not known until a worker has run a model, rendered a document and
-- uploaded it. See migrations/V1.2.6__assessment_generator.sql.
-- ============================================================================
