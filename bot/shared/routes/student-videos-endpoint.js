/**
 * Student Videos Flow Endpoint — v2.0 (bd-1598 follow-up)
 *
...
async function fetchDone(filter = {}) {
  // bd-2318: hide duplicates. 85 videos were identified as re-uploads of
...
function deliverVideoAsync(flowToken, row) {
  (async () => {
    ...
    if (resolved.kind === 'child') {
      ...
      // The reachable point where the child's Student-Videos
    ...
    try {
      ...
      // bd-2310: record the delivery so we can measure how popular the feature
      ...
      // bd-2308: offer the video's quiz 3 s after delivery.
      //
