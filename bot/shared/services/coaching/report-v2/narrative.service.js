...
function buildPrompt(analysis, { transcript, trend = [], language, teacherName }) {
  ...
  const domainScoresBlock = (isFico && a.domains && typeof a.domains === 'object')
    ? Object.keys(FICO_DOMAIN_LABELS).filter((k) => a.domains[k]).map((k) => {
        ...
        // v4 scores 0-2, so a teacher at FULL marks would otherwise be described to
        // the model as "2/4" — half. Only the applicable-aware computeScores knows
        // the real ratio, and it records it as `indicators_applicable`. Without that
        // field we are on a pre-cutover analysis where 4 is correct — deriving from
        // `indicators.length` instead is wrong whenever the stored list is partial.
