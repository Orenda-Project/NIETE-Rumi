...
async function findRender(segmentId, lang, tv) {
  const { data, error } = await supabase
    ...
    // `render_degraded` (bd-oak77.14): every teacher after the first is served entirely from this
    // row, so the honesty line on her caption has nowhere else to come from — an unread column
    // would tell the teacher who waited and none of the ones who tapped later, about one file.
