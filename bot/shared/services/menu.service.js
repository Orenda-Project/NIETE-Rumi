...
class MenuService {
  ...
  static async _handleLessonPlanningChoice(userId, sessionId, from, language, reason = 'menu') { // `reason`: which door — see lp-browse-entry.service
    ...
    // One door, one copy — shared with the bare "lp" command, the lesson_plan
    // intent (text + voice) and the Oxbridge fallback tap, so the entry points
    // cannot drift apart again (bd-72dth). Caps + bilingual copy are pinned in
    // tests/lp-v8/bd-hgwfo-gamma-door.test.js.
    ...
    // Fallback — only when no Flow is provisioned or the send failed. The topic she
    // types then reaches handleLessonPlanRequest, which tries the Flow again and,
    // when LP_612_ROUTE_ALL is off, still generates through Gamma. That is the
    // rollback lever: this branch GATES the Gamma path, it does not delete it.
