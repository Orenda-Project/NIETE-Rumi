 ...
 */
// Grade-band pedagogy menus. PRIMARY-grade strategies (thumbs up/down, sensory
// hand-signals, chant-and-repeat, stand-up-and-move sorting) are
// age-inappropriate for Grades 6-12 and were flagged by a partner content
// reviewer against secondary-grade samples. The prompt now BINDS the LLM to
// pick strategies ONLY from the menu that matches the requested grade band.
function pedagogyMenuFor(grade) {
  const g = typeof grade === 'string' ? parseInt(grade.match(/\d+/)?.[0] || '0', 10) : Number(grade) || 0;
  if (g >= 1 && g <= 5) {
    return {
      band: 'PRIMARY (Grades 1-5)',
      menu: [
        'Manipulatives (counters, blocks, real objects)',
        'Sensory activities (touch, movement, colour sorting)',
        'Call-and-response chants; hand signals (thumbs up / down)',
        'Song / rhyme; picture-story sequencing',
        'Choral reading; large-print flashcards',
      ],
    };
  }
  if (g >= 6 && g <= 8) {
    return {
      band: 'MIDDLE (Grades 6-8)',
      menu: [
        'Pair work; think-pair-share (short-form)',
        'Gallery walks; poster summaries',
        'Entry/exit tickets; short written reflections',
        'Structured note-taking (Cornell, T-chart)',
        'Small-group problem sets with reporter roles',
      ],
    };
  }
  if (g >= 9 && g <= 12) {
    return {
      band: 'SECONDARY (Grades 9-12)',
      menu: [
        'Think-pair-share (extended); pair-then-write',
        'Jigsaw (expert groups + home groups)',
        'Structured academic debate; four-corners',
        'Socratic seminar; teacher-led questioning ladder',
        'Independent practice with worked-example scaffolds',
        'Peer review of short written responses',
      ],
    };
  }
  return null; // unknown grade — do not inject a menu
}

function buildLessonPlanPrompt({ language, grade, subject } = {}) {
  // ── Content-quality guardrails ─────────────────────────────────────────────
  // Baked into the base prompt so every LP generated by this service inherits
  // them. Five explicit rules, ordered by severity:
  //   1. NBF-source-fidelity — anchor every activity/example to a specific
  //      NBF chapter + page (not generic subject knowledge).
  //   2. Grade-band-aware pedagogy — pick from the ONE menu that matches the
  //      requested grade band; NO primary-grade strategies for Grades 6-12.
  //   3. Concise + practical — length caps per section, teacher-usable in
  //      40 minutes; do NOT pad.
  //   4. Formatting integrity — Unicode-safe; no raw math markup / LaTeX /
  //      subscript-superscript glyphs; write "H2O" as "H₂O" only if you can
  //      guarantee the renderer supports it (default: spell out "H two O" or
  //      "water molecule").
  //   5. Learning-outcome alignment — every example + assessment must trace
  //      back to a stated chapter LO, cited inline in the LP.
  const guardrails = `CONTENT-QUALITY GUARDRAILS — apply to every lesson plan:
1. NBF SOURCE FIDELITY. Every activity, example, and assessment MUST be anchored to a specific NBF (National Book Foundation) chapter/section and page number. Cite the page reference inline (e.g. "Example 2, p.7"). Do NOT invent generic subject content that could apply to any textbook.
2. GRADE-BAND-AWARE PEDAGOGY. Pick engagement strategies ONLY from the menu that matches the requested grade band (see PEDAGOGY MENU below). Do NOT use primary-grade strategies (thumbs up/down, hand signals, sensory hand-on-head / hand-out-flat sorting, call-and-response chants, stand-up-and-move body sorting) in Grades 6-12. Secondary students find them condescending.
3. CONCISE + PRACTICAL. This is a 40-minute lesson for a real teacher. Length caps: SLO ≤ 3 sentences; I-Do ≤ 6 bullets; We-Do ≤ 5 bullets; You-Do ≤ 4 bullets; CFU + Exit Ticket ≤ 3 sentences each. Do NOT pad. If you cannot fit the content in 40 minutes, cut scope, not depth.
4. FORMATTING INTEGRITY. Plain text only. No raw LaTeX, no unescaped math markup, no exotic Unicode subscripts/superscripts. If you need to write a chemical formula, spell it out ("carbon dioxide" not "CO₂") unless the renderer is verified Unicode-safe. Use ASCII arrows (->), ASCII multiplication (x or *), plain hyphens.
5. LEARNING-OUTCOME ALIGNMENT. Every example, activity, and assessment MUST trace back to a stated chapter Learning Outcome (LO). Cite the LO inline in the LP (e.g. "aligned to LO 1.1.a: converts between currencies using a rate").`;

  const menu = pedagogyMenuFor(grade);
  const pedagogyBlock = menu
    ? `PEDAGOGY MENU FOR GRADE BAND: ${menu.band}. Pick from these ONLY:\n${menu.menu.map((m, i) => `  ${i + 1}. ${m}`).join('\n')}`
    : `PEDAGOGY MENU: no grade specified — default to Grades 6-8 middle-school strategies (pair work, gallery walks, entry/exit tickets, structured note-taking, small-group problem sets).`;
...
module.exports = {
  ...
  pedagogyMenuFor,         // grade-band pedagogy menus
