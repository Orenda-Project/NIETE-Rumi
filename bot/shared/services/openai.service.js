...
4. GUIDE teacher training - Modules and exams. Type "/training" to continue.
...
CRITICAL: NEVER say "I can't do that" or "I'm unable to" for any of the capabilities above.
...
- False promises destroy user trust - this is a critical rule`;
   ...
   * @param {object} [opts] - WHO is speaking. Backwards compatible — every
   *   existing caller omits this and gets the teacher prompt unchanged.
   ...
   * lesson plans, presentations, coaching feedback and training), and
...
4. ٹیچر ٹریننگ میں رہنمائی کریں - /training ٹائپ کریں۔
...
اہم: اوپر کی صلاحیتوں کے لیے کبھی نہ کہیں "میں یہ نہیں کر سکتی"۔
...
4. GUIDE teacher training - Type /training to continue.
...
Keep your responses relatively short as they will be sent via WhatsApp messages.`;
   ...
   * @param {object} [opts] - passed straight through to
  ...
  async getResponseWithFormat(userMessage, userId, format, language, firstName = null, featureContext = null, opts = {}) {
    try {
      ...
      // A child gets a child-shaped apology, not the teacher wording below —
      // that wording is unchanged, this is additive.
