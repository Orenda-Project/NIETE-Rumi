...
describe('options a child can actually READ (operator round 6)', () => {
  ...
  // bd-2358: both tests below asserted the body spell-out at the TITLE cap.
  // The operator's requirement was that a child can read the whole option —
  // "5 red pencils and 10 blu" was unreadable. That is now met by the row
  // DESCRIPTION, which holds 72 characters, so the body no longer repeats
  // options it can carry. What must still hold is that the full option reaches
  // the child somewhere; that is what these now assert.
  test('options too long for a 24-char row title survive intact for the row', () => {
    const ask = render.build(longOpts()).find((m) => m.role === 'ask');
    expect(ask.kind).toBe('list');
    // handed to the sender whole — it becomes the row description there
    expect(ask.options).toContain('5 red pencils and 10 blue pencils');
    expect(ask.options).toContain('23 blue pencils, and 19 red pencils');
    // and are NOT also printed in the body
    expect(ask.body).not.toContain('A. 5 red pencils');
  });

  test('a picture question with long options keeps them readable too', () => {
  ...
  test('an option too long even for the description is still spelled out', () => {
    const huge = 'z'.repeat(90);
    const ask = render.build({ ...longOpts(), option_a: huge }).find((m) => m.role === 'ask');
    // bd-2359 shuffles display order, so the letter follows where the option
    // now sits — what must hold is that it is lettered and spelled out in full.
    const letter = render.optionLetter(ask.options.indexOf(huge));
    expect(ask.body).toContain(`${letter}. ${huge}`);
  });
