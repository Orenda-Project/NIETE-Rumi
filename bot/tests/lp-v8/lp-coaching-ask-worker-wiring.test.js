'use strict';
/**
 * the coaching-ask handler is registered on the worker that sweeps.
 *
 * The sweeper claims only kinds something in its process registered. The REAL
 * `startWorker()` boots here (boundaries mocked exactly as the nudge-core wiring
 * suite does); the assertion is that `coaching_after_lp` is registered, with
 * the send handler, on the service that owns the sweep — and on no other.
 */
...
describe('coaching_after_lp is registered where the sweep runs', () => {
  test('flag on, main queue: registered once, with send and a prepare step', async () => {
    await boot({ TEACHER_NUDGES_ENABLED: 'true' });
    const calls = mockRegister.mock.calls.filter((c) => c[0] === 'coaching_after_lp');
    expect(calls).toHaveLength(1);
    const Ask = require('../../shared/services/nudges/lp-coaching-ask.service');
    expect(typeof calls[0][1]).toBe('function');
    expect(typeof (calls[0][2] || {}).prepare).toBe('function');
    expect(Ask.KIND).toBe('coaching_after_lp');
  });

  test('a video-only worker never loads the ask', async () => {
    await boot({ TEACHER_NUDGES_ENABLED: 'true', WORKER_QUEUES: 'video' });
    expect(mockRegister.mock.calls.filter((c) => c[0] === 'coaching_after_lp')).toHaveLength(0);
  });
});
