...
## F5 · BIOLOGY, CHEMISTRY AND GENERAL SCIENCE — the descriptive-science notes

*(This section is in the science brief only. This family's defects are overwhelmingly SCHEMA and
unworded questions, not maths: `MATH_LEAK` occurred once in the whole measured corpus, so there
is no maths block here.)*

**① Equations and structures belong in a DIAGRAM SPEC, not in prose LaTeX.** Use the
`chem_equation` spec for reactions and `molecule` for structures — they typeset properly and
translate:

```json
{"type": "diagram", "id": "dg-1",
 "spec": {"type": "chem_equation", "equation": "2H2 + O2 -> 2H2O",
          "title": "Hydrogen burns in oxygen", "caption": "…"}}
```
Inside that spec, write any stray symbol as Unicode (`→`, `⇌`, `°C`, `H₂O`), never `$…$`.

**② A real textbook figure needs its `legend`.** The printed labels on a crop are baked pixels and
vanish at phone scale, so a `textbook_figure` without `legend` gives the pupil nothing to point at.

**③ Two diagrams must not say the same thing** (`DUPLICATE_DIAGRAM`), and a diagram's labels must
not collide (`DIAGRAM_OVERLAP`) — keep label text short inside specs.

**④ Every question is a full sentence** (`UNWORDED_Q`) — this family's second-largest defect
class. "Observation vs hypothesis?" is not a question; "State one qualitative observation from the
malaria case, and say why it is qualitative." is.
