...
set -u
cd "$(dirname "$0")/.." || exit 1
# The installer exits early under CI=… on purpose (CI clones never commit), and
# GitHub Actions sets CI=true — so the very job that runs this suite would make
# every install case fail. These cases test the installer's behaviour on a
# developer machine; give them that environment.
