...
describe('applyLpFidelity — FICO Section B from measured fidelity (P4.1 / D27)', () => {
  ...
  // 47.4% of sessions, and one of its indicators credited a quarter of those
  ...
  test('fidelity unusable (ok, pct null) → Section B is not scored and leaves the total', () => {
    ...
    expect(a.domains.lesson_plan_fidelity.domain_score).toBe(BASE_B); // row unchanged
    expect(a.domains.lesson_plan_fidelity.fidelity_derived).toBeFalsy();
    expect(a.scores.overall_marks).toBe(BASE_TOTAL - BASE_B);
    expect(a.scores.overall_max_marks).toBe(fico.maxMarks - MAX_B);
  });
