 ...
 * recording's audio length, and the orchestrator receives it together with the photo evidence the analysis stored.
 */
const mockSelects = [];
...
test('the default loader selects audio_duration_seconds; it reaches the orchestrator with the stored photo evidence', async () => {
  ...
  expect(seen[0]).toMatchObject({ transcript: '[00:10] t', audioDurationSeconds: 1234, photoEvidence: [{ n: 1, kind: 'board', visible_text: 'x' }] });
