 ...
 * mock is the model client (network boundary). The linked lesson plan + a canned response
 * without `fidelity_to_lesson_plan` forces the second, gpt-4o-mini fidelity call, which must
 * still receive a plain-string user message (the eval harness had to special-case this).
 */
...
describe('bd-8s2xb — analyzePedagogy attaches images to the scoring call only', () => {
  ...
  test('T4: both mode, 3 images, LP linked → call 1 has text + 3 image parts; call 2 (fidelity) is plain text', async () => {
    ...
    const fidelity = calls[1];
    expect(fidelity.model).toBe('gpt-4o-mini');
    expect(typeof fidelity.messages[0].content).toBe('string');
  });
