...
describe('report-generator → transcript quiz offer', () => {
  ...
  test('a scheduled offer suppresses BOTH the older quiz trigger and the feature-link ask, and neither is deleted', () => {
    const s = src('shared/services/coaching/report-generator.service.js');
    // This branch ships onto production `main`, where the older quiz trigger is
    // live. It is gated by the same boolean, never removed — flag off, the
    // offer service returns false and both blocks run exactly as they do today.
    expect(s).toMatch(/await this\.offerQuizAfterReport\(/);
    expect(s).toMatch(/quizOfferScheduled = await this\.scheduleTranscriptQuiz\(/);
    expect(s).toMatch(/if \(!quizOfferScheduled\) try \{[\s\S]{0,600}offerQuizAfterReport\(/);
    expect(s).toMatch(/if \(!quizOfferScheduled\) try \{[\s\S]{0,600}FeatureLinkerService\.suggestNext\(/);
  });
});

// ─── 2. survey answer brings the offer forward ───────────────────────────────
// NOT ON THIS BRANCH. The early trigger lives in the coaching-survey lane's
// coaching-feedback.service.js, which production `main` does not carry; pulling
// it in would import that whole lane under the quiz lane's name. On `main` the
// offer therefore fires on its own delayed job only. The test is kept, skipped,
// so it comes back the day that lane lands rather than being quietly deleted.
describe.skip('coaching survey answer triggers the offer early (coaching-survey lane, not on main)', () => {
  beforeEach(() => jest.resetModules());
